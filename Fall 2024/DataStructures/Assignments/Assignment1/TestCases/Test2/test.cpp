#include "pch.h"
#include "../test_Assignment1/PlagiarismChecker.cpp"
#include <fstream>
#include <string>
#include <math.h>
...
TEST(PC, RemovePunctucationMarks)
{
	...
	char expected[] = "a e i o u";  // Explain
	char expected1[] = "vowels a e i o u";
...
TEST(PC, PlagCheck1) {
	...
	EXPECT_NEAR(similarityIn(2, 1), 43.00, 0.3);
	EXPECT_NEAR(similarityIn(1, 2), 43.00, 0.3);
}
...
TEST(PC, MaxfileRead) {
	...
	convertUpperToLowerCase();
	char expected[] = "aeiou";
	EXPECT_EQ(0, strcmp(getText(1000), expected));		//This should return the content of doucment 1000
...
TEST(PC, HalfFilesRead) {
	...
	EXPECT_EQ(0, strcmp(getText(9), expected));		//This should return the content of doucment 9
